...
- PowerShell 없는 훅 설치·복구는 자동 시험을 통과했지만, 이전 실행 정책 문제가 발생한 별도
  Windows에서 최종 빌드를 다시 검증하지 못했습니다. 설치 후 앱의 `7/7` 표시와 `/hooks`의
  7개 항목을 확인하세요. 이 제한은 키·노브·LED 설정 기능에는 영향을 주지 않습니다.
