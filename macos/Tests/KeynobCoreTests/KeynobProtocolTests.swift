import Foundation
import XCTest
@testable import KeynobCore

final class KeynobProtocolTests: XCTestCase {
    func testReadLayerRequestMatchesWindowsProtocol() throws {
        let bytes = [UInt8](try KeynobReportCodec.readLayerRequest(layer: 2))
    ...
    func testCurrentSlotValidationMatchesWindowsRules() {
        ...
        XCTAssertTrue(KeynobReportCodec.isValidCurrentReport(report, layer: 3, slot: 21))
        XCTAssertFalse(KeynobReportCodec.isValidCurrentReport(report, layer: 2, slot: 21))

        var invalid = [UInt8](report)
        invalid[13] = 0x01
        XCTAssertFalse(KeynobReportCodec.isValidCurrentReport(Data(invalid), layer: 3, slot: 21))
    }
    ...
    func testLayerParserRejectsDuplicateSlots() throws {
        ...
        XCTAssertThrowsError(try KeynobReportCodec.parseLayer(layer: 1, reports: reports)) { error in
            XCTAssertEqual(error as? KeynobProtocolError, .duplicateSlot(1))
        }
    }

    func testLayerParserAcceptsSupportedLayout() throws {
        let reports = (1...25).map { makeValidSlot(layer: 1, slot: $0, codes: [0x04]) }
        let snapshot = try KeynobReportCodec.parseLayer(layer: 1, reports: reports)
    ...
    func testLEDParserReadsTwelveColors() throws {
        ...
        let snapshot = try KeynobReportCodec.parseLED(report: Data(report))
    ...
    func testBackupRejectsChecksumTampering() throws {
        let url = FileManager.default.temporaryDirectory
            .appendingPathComponent("keynob-xctest-\(UUID().uuidString).json")
        ...
        let layers = try (1...3).map { layer in
            try KeynobReportCodec.parseLayer(
                layer: layer,
