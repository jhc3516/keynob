...
private final class KeynobAppDelegate: NSObject, NSApplicationDelegate {
...
@main
struct KeynobApp: App {
    @NSApplicationDelegateAdaptor(KeynobAppDelegate.self) private var appDelegate

    var body: some Scene {
        Window("Keynob", id: "studio") {
...
private struct KeynobInputSelector: View {
...
private struct ContentView: View {
    ...
    private let service = KeynobConfigurationService()
    ...
    private var header: some View {
        HStack {
            VStack(alignment: .leading, spacing: 3) {
                Text("Keynob").font(.largeTitle.bold())
                Text("AI 도구를 손끝에서 · ChatGPT 앱과 Codex CLI").foregroundStyle(.secondary)
            }
            Spacer()
        ...
        HStack(alignment: .top, spacing: 20) {
            VStack(alignment: .leading, spacing: 12) {
                Picker("레이어", selection: $selectedLayer) {
                    ForEach(Array(KeynobIdentity.layerIDs), id: \.self) { Text("Layer \($0)").tag($0) }
                ...
                GroupBox("Codex CLI 전용 실행기") {
                    VStack(alignment: .leading, spacing: 10) {
                        ...
                        Button("작업 폴더를 골라 Codex CLI 열기") { terminalLauncher.chooseFolderAndLaunch() }
                            .buttonStyle(.borderedProminent)
                        Text("Keynob가 연 Terminal 창만 Codex CLI 범위로 인식합니다. 첫 실행에는 Terminal 자동화 권한 확인이 나타날 수 있습니다.")
                            .font(.caption).foregroundStyle(.secondary)
                    }.padding(8).frame(maxWidth: .infinity, alignment: .leading)
                }
