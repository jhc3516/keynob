...
private func makeValidSlot(layer: Int, slot: Int, codes: [UInt8]) -> Data {
    var report = [UInt8](repeating: 0, count: KeynobReportCodec.inputReportLength)
...
private func makeSnapshot() throws -> FullDeviceSnapshot {
    ...
    let layers = try KeynobIdentity.layerIDs.map { layer -> LayerSnapshot in
        let reports = (1...KeynobIdentity.slotCount).map { makeValidSlot(layer: layer, slot: $0, codes: [0x04]) }
...
private func runSelfTest() throws -> SelfTestSummary {
    ...
    invalid[13] = 0x01
    try check(!KeynobReportCodec.isValidCurrentReport(Data(invalid), layer: 3, slot: 21), "invalid_report")
    ...
    let backupURL = FileManager.default.temporaryDirectory.appendingPathComponent("keynob-self-test-\(UUID().uuidString).json")
    ...
    let settingsDirectory = FileManager.default.temporaryDirectory
        .appendingPathComponent("keynob-settings-self-test-\(UUID().uuidString)")
    ...
    do {
        try KeynobConfigurationService.requireUnchanged(snapshot.led, current: statusLED)
        throw SelfTestFailure(check: "stale_led_rejected")
    } catch ConfigurationServiceError.staleConfiguration { passed += 1 }
    do {
        try KeynobConfigurationService.requireUnchanged(encoded, current: disabledReport)
        throw SelfTestFailure(check: "stale_slot_rejected")
    } catch ConfigurationServiceError.staleConfiguration { passed += 1 }
    try KeynobConfigurationService.requireUnchanged(snapshot.led, current: snapshot.led)
    passed += 1
    try KeynobConfigurationService.requireUnchanged(nil, current: snapshot.led)
    passed += 1
    ...
    try check(AppTargetPolicy.scope(
        ...
        focusedWindowTitle: "Codex CLI - Keynob [mac-test]") == .codexCLI, "dedicated_cli_target")
    try check(AppTargetPolicy.scope(bundleIdentifier: "com.apple.Terminal", focusedWindowTitle: "zsh") == nil &&
              ...
              AppTargetPolicy.scope(bundleIdentifier: "com.apple.Terminal", focusedWindowTitle: "Codex CLI - Keynob") == nil &&
...
private func parseLED(mode: String, colorsHex: String) throws -> LEDSnapshot {
    guard let modeValue = UInt8(mode), modeValue <= 5,
          let bytes = Data(hex: colorsHex), bytes.count == 36 else { throw KeynobProtocolError.invalidLEDState }
    let raw = [UInt8](bytes)
...
do {
    ...
    case "read-layer":
        guard arguments.count == 2, let layer = Int(arguments[1]) else { throw KeynobProtocolError.invalidLayer(-1) }
    ...
    case "program-slot":
        ...
        guard try report(in: before, layer: layer, slot: slot) == expected else { throw KeynobHIDError.bridge(code: 7, message: "stale_device_state") }
    ...
    case "program-led":
        ...
        let store = backupStore(arguments.count == 6 ? arguments[5] : nil)
        let before = try hid.readFullSnapshot(); _ = try store.save(before)
        guard before.led == expected else { throw KeynobHIDError.bridge(code: 7, message: "stale_device_state") }
    ...
    case "restore-backup":
        ...
        let service = KeynobConfigurationService(hid: hid, backupStore: backupStore(path))
