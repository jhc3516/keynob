...
public enum KeynobProtocolError: Error, Equatable, LocalizedError {
...
public enum KeynobReportCodec {
    ...
    public static func parseLayer(layer: Int, reports: [Data]) throws -> LayerSnapshot {
        ...
        guard reports.count == KeynobIdentity.slotCount else {
            throw KeynobProtocolError.invalidReportLength(reports.count)
        }

        var slotsByNumber: [Int: Data] = [:]
        for report in reports {
            guard report.count == inputReportLength else {
                throw KeynobProtocolError.invalidReportLength(report.count)
            }
            let bytes = [UInt8](report)
            guard bytes[0] == 0x03, bytes[1] == 0xFA, bytes[3] == UInt8(layer) else {
                throw KeynobProtocolError.invalidReportHeader
            }
            let slot = Int(bytes[2])
            guard (1...KeynobIdentity.slotCount).contains(slot) else {
                throw KeynobProtocolError.invalidSlot(slot)
            }
            guard slotsByNumber[slot] == nil else {
                throw KeynobProtocolError.duplicateSlot(slot)
            }
        ...
        let slots = (1...KeynobIdentity.slotCount).map { slot in
    ...
    public static func isValidReplacementReport(_ report: Data, layer: Int, slot: Int) -> Bool {
        guard report.count == inputReportLength,
              KeynobIdentity.layerIDs.contains(layer),
              (1...KeynobIdentity.slotCount).contains(slot) else { return false }
        let bytes = [UInt8](report)
        guard bytes[0] == 0x03,
              bytes[1] == 0xFA,
              bytes[2] == UInt8(slot),
