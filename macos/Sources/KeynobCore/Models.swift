...
public struct HIDDeviceDescriptor: Equatable, Sendable {
    ...
    public var isSupportedIdentity: Bool {
        vendorID == KeynobIdentity.vendorID &&
            productID == KeynobIdentity.productID &&
            usagePage == KeynobIdentity.usagePage &&
            interfaceNumber == KeynobIdentity.interfaceNumber
    }
}
...
public struct DeviceSnapshot: Codable, Equatable, Sendable {
    ...
    public init(
        ...
        self.vendorID = String(format: "%04X", KeynobIdentity.vendorID)
        self.productID = String(format: "%04X", KeynobIdentity.productID)
        self.usagePage = String(format: "%04X", KeynobIdentity.usagePage)
        self.interfaceNumber = KeynobIdentity.interfaceNumber
        self.serial = serial
