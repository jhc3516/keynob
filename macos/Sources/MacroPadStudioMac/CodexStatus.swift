...
private final class CodexStatusSocketServer: @unchecked Sendable {
    ...
    private let queue = DispatchQueue(label: "io.github.jhc3516.macropad.status")
...
private struct CodexHookInstaller {
    ...
    private var packagedHelperURL: URL? {
        Bundle.main.executableURL?.deletingLastPathComponent().appendingPathComponent("macropad-status-hook")
    }
    ...
    private func backupHooksIfPresent() throws {
        ...
        let backup = codexDirectory.appendingPathComponent("hooks.json.macropad-backup-\(stamp)")
