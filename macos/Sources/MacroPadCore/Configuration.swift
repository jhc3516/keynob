...
public enum DeviceConfigurationCodec {
    public static func decodeShortcut(report: Data, layer: Int, slot: Int) throws -> DeviceShortcut {
        guard MacroPadReportCodec.isValidCurrentReport(report, layer: layer, slot: slot) else {
    ...
    public static func encodeShortcut(_ shortcut: DeviceShortcut, layer: Int, slot: Int) throws -> Data {
        guard MacroPadIdentity.layerIDs.contains(layer), EditableInput.forSlot(slot) != nil else {
        ...
        guard MacroPadReportCodec.isValidReplacementReport(report, layer: layer, slot: slot) else {
            throw MacroPadProtocolError.incompatibleSlot(layer: layer, slot: slot)
        }
        return report
    }
...
public struct DeviceBackupStore: Sendable {
    ...
    public static func validate(_ backup: DeviceBackup) throws {
        ...
        guard backup.schemaVersion == 1,
              ...
              Set(snapshot.layers.map(\.layer)) == Set(MacroPadIdentity.layerIDs),
              snapshot.led.mode <= 5, snapshot.led.colors.count == 12 else {
            throw DeviceBackupError.invalidMetadata
        }
        for layer in snapshot.layers {
            guard layer.slots.count == MacroPadIdentity.slotCount,
                  Set(layer.slots.map(\.slot)) == Set(1...MacroPadIdentity.slotCount) else {
                throw DeviceBackupError.invalidMetadata
            }
            for slot in layer.slots {
                guard let data = Data(hex: slot.hex), data.count == MacroPadReportCodec.inputReportLength,
