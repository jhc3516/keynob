...
public struct MacroPadConfigurationService: Sendable {
    public let hid: MacroPadHID
    public let backupStore: DeviceBackupStore

    public init(hid: MacroPadHID = MacroPadHID(), backupStore: DeviceBackupStore = DeviceBackupStore()) {
        self.hid = hid
    ...
    public func applyReport(
        ...
        guard MacroPadReportCodec.isValidReplacementReport(
            replacement, layer: layer, slot: input.slot) else {
            throw MacroPadProtocolError.incompatibleSlot(layer: layer, slot: input.slot)
        }
    ...
    private func reservedSlotsMatch(_ lhs: FullDeviceSnapshot, _ rhs: FullDeviceSnapshot) -> Bool {
        ...
        for layer in MacroPadIdentity.layerIDs {
            for slot in 1...MacroPadIdentity.slotCount where !editable.contains(slot) {
    ...
    private func fullStateMatches(_ lhs: FullDeviceSnapshot?, _ rhs: FullDeviceSnapshot) -> Bool {
        ...
        for layer in MacroPadIdentity.layerIDs {
            for slot in 1...MacroPadIdentity.slotCount {
                guard (try? report(in: lhs, layer: layer, slot: slot)) ==
                    (try? report(in: rhs, layer: layer, slot: slot)) else { return false }
            }
        }
