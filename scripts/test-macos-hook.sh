...
temporary_root="$(mktemp -d /private/tmp/keynob-status-test-XXXXXX)"
...
if [[ $# == 1 ]]; then
    ...
    swift build --package-path "$package_path" --configuration debug --product keynob-status-hook
    binary_path="$(swift build --package-path "$package_path" --configuration debug --show-bin-path)/keynob-status-hook"
fi
...
printf '%s' '{"hook_event_name":"UserPromptSubmit","session_id":"session-1","turn_id":"turn-1","failed":true,"prompt":"must-not-leave-helper"}' |
    KEYNOB_TEST_STATUS_SOCKET="$socket_path" CODEX_KEYBOARD_INSTANCE_ID="mac-test-instance" "$binary_path"
