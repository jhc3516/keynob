...
using CodexKeyboardStudio;
using CodexKeyboardStudio.Models;
using CodexKeyboardStudio.Services;
using Key = System.Windows.Input.Key;

var testRoot = Path.Combine(Path.GetTempPath(), $"CodexKeyboardStudio.SettingsTests-{Guid.NewGuid():N}");
...
try
{
    ...
    Assert(StartupRegistrationService.BuildCommand(@"C:\Portable App\CodexKeyboardStudio.exe") ==
        "\"C:\\Portable App\\CodexKeyboardStudio.exe\" --background", "startup_command_is_quoted");
    ...
    Assert(ForegroundAppDetector.Classify("WindowsTerminal", null, "Codex CLI - MacroPad Studio") == "codex_cli", "classify_marked_terminal");
    Assert(ForegroundAppDetector.Classify("powershell", null, "Codex CLI - MacroPad Studio - project") == "codex_cli", "classify_marked_powershell");
    ...
    Assert(ForegroundAppDetector.Classify("notepad", null, "Codex CLI - MacroPad Studio") is null, "reject_unapproved_process");
    ...
    Assert(hookUninstall.Ok && !File.Exists(installedHookClient) &&
        ...
        "native_hook_uninstall_is_scoped_to_macropad_hooks");
